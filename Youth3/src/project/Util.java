...
public class Util {
  ...
  /***
	 ...
	 *         cache) with the specified properties
	 */
public static Iterable<Entity> listEntities(String kind, String searchBy, String searchFor,int offset) {
  logger.log(Level.INFO, "Search entities based on search criteria");
  Query query = new Query(kind);
  if (searchFor != null && !"".equals(searchFor)) {
    query.addFilter(searchBy, FilterOperator.EQUAL, searchFor);
  ...
  //fetch results from datastore based on offset and page size of 10
  FetchOptions fetchOptions = FetchOptions.Builder.withLimit(10);
  return pq.asQueryResultList(fetchOptions.offset(offset));
}
  ...
  public static String writeJSON(Iterable<Entity> entities) {
    ...
    for (Entity result : entities) {
     ...
     for (String key : properties.keySet()) {
    	 if(properties.get(key) instanceof Text){
    		 //System.out.println("Found content:"+properties.get(key));
    ...
    sb.append("]}");//System.out.println("response length:"+sb.length());
    //System.out.println("Writing resp:"+sb.toString());
    //System.out.println("Response Size:"+sb.toString().getBytes().length);
    return sb.toString();
  }
